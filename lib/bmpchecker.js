...
class BMPChecker{
  ...
  check_bmp(data){
    ...
    // TODO : optional check color table
  ...
  check_bmp_file_header(data){
    ...
    // If in verbose write the bitmap header
    if(this.opts.verbose){
      console.log('bitmap_header:');
      console.log(this.bitmap_header);
    }
  ...
  check_data(data){
    if(this.bitmap_type == 'BITMAPINFOHEADER' && 
        this.infoHeaderCompressionTypes.get(
          this.bitmap_dib_header.compression_method) == 'BI_RGB'){

      /*
        Calc row size
                     |  BitsPerPixel * ImageWidth + 31|
          row_size = |--------------------------------| * 4
                     |_             32               _|
      */

      let inner_adding = this.bitmap_dib_header.bits_per_pixel * this.bitmap_dib_header.width + 31;
      if(this.opts.verbose)
        console.log(`inner_adding: ${inner_adding}`);
      let row_size = Math.floor(inner_adding / 32) * 4;
      if(this.opts.verbose)
        console.log(`row_size: ${row_size}`);
      let data_size = row_size * Math.abs(this.bitmap_dib_header.height);
      if(this.opts.verbose)
        console.log(`data_size: ${data_size}`);
      let total_size = this.bitmap_header.offset_to_data + data_size;
      
      if(!this.check_condtion(total_size == data.length, 
        'Total data size is as expected', 
        `Expected total data size as ${total_size} found ${data.length}`))
        return false;

      if(this.bitmap_dib_header.image_size != 0){
        if(!this.check_condtion(data_size == this.bitmap_dib_header.image_size, 
          'Check in case image_size in dib is not 0 if size as expected', 
          `Expected size in dib ${data_size} found ${this.bitmap_dib_header.image_size}`))
          return false;
      }
    } else if (this.bitmap_type == 'BITMAPINFOHEADER') {
      if(!this.check_condtion(false, '', 
      `BITMAPINFOHEADER compression ${this.bitmap_dib_header.compression_method}
       not supported yet`))
       return false;
    } else {
      if(!this.check_condtion(false, 
        '', `${this.bitmap_type} not supported data check`))
        return false;
    }
  ...
  read_BITMAPINFOHEADER(data){
    ...
    if(this.opts.verbose){
      ...
      console.log(this.bitmap_dib_header);
    }
